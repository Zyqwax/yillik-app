...
export default function AdminVitrinClient({ initialPhotos, user }: { initialPhotos: PhotoType[], user: UserType }) {
  ...
  const toggleSetting = async (key: 'uploadEnabled' | 'deleteEnabled', current: boolean) => {
  ...
  const handleToggleHide = async (id: string) => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setAllPhotos(prev => prev.map(p => p.id === id ? { ...p, isHidden: data.isHidden } : p));
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const handleToggleFavorite = async (id: string) => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setAllPhotos(prev => prev.map(p => p.id === id ? { ...p, isAdminFavorite: data.isAdminFavorite } : p));
      }
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <div className={styles.container}>
      ...
      <div style={{
        ...
        <span style={{ fontWeight: 600, opacity: 0.8, fontSize: '0.9rem' }}>⚙️ Site Ayarları:</span>
        ...
        <button
          ...
          style={{
            ...
            fontWeight: 600,
            fontSize: '0.85rem',
            background: uploadEnabled ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)',
            color: uploadEnabled ? '#4ade80' : '#f87171',
            transition: 'all 0.2s',
          }}
        >
          {uploadEnabled === null ? '...' : uploadEnabled ? '📤 Yükleme: Açık' : '📤 Yükleme: Kapalı'}
        </button>
        ...
        <button
          ...
          style={{
            ...
            fontWeight: 600,
            fontSize: '0.85rem',
            background: deleteEnabled ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)',
            color: deleteEnabled ? '#4ade80' : '#f87171',
            transition: 'all 0.2s',
          }}
        >
          {deleteEnabled === null ? '...' : deleteEnabled ? '🗑️ Silme: Açık' : '🗑️ Silme: Kapalı'}
        </button>
      </div>
